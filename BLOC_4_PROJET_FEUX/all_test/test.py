...
# Importe de la fonction à tester
...
def test_compile_meteo_data():
    ...
    # On simule l'exécution de la fonction sur ce fichier
    all_files = [file_path]
    list_of_dfs = []
    ...
    for file in all_files:
        try:
            # Premier essai avec le sépérateur ;
            df = pd.read_csv(file, sep=';', on_bad_lines='skip', engine='python')

            # Si le DataFrame n'a qu'une seule colonne, essai avec ,
            if df.shape[1] == 1:
                df = pd.read_csv(file, sep=',', on_bad_lines='skip', engine='python')

            # Si toujours 1 seule colonne, essai avec tabulation
            if df.shape[1] == 1:
                df = pd.read_csv(file, sep='\t', on_bad_lines='skip', engine='python')
