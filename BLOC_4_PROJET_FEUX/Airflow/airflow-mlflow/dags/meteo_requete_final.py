...
def get_meteo(ti, **kwargs):
    ...
    # Récupérer les stations ouvertes du département 20 (Corse)
    params_stations = {'id-departement': dep, 'parametre': 'temperature'}
    stations_req = requests.get(url_stations, headers=headers, params=params_stations)
    print("Stations →", stations_req.status_code)

    if stations_req.status_code != 200:
        raise Exception(f"Erreur récupération stations: {stations_req.status_code}")

    stations_df = pd.DataFrame(stations_req.json())
    stations_df['posteOuvert'] = stations_df['posteOuvert'].astype(bool)
    stations_df = stations_df[stations_df['posteOuvert']]

    id_stations = stations_df['id'].tolist()
    print(f"➡ {len(id_stations)} stations ouvertes trouvées")

    # Période demandée : uniquement depuis le 1er juillet 2025
    date_debut = "2025-07-01T00:00:00Z"
    date_fin = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")

    base_cmd_url = "https://public-api.meteofrance.fr/public/DPClim/v1/commande-station/quotidienne"
    base_file_url = "https://public-api.meteofrance.fr/public/DPClim/v1/commande/fichier"

    all_paths = []

    for station_id in id_stations:

        # ---- ⚠ CRUCIAL : initialiser id_cmde à None ----
        id_cmde = None
        ...
        # --- 1) ENVOI DE LA COMMANDE + POLLING ---
        max_retries = 20
        wait = 5
        ...
        for attempt in range(max_retries):
            r = requests.get(base_cmd_url, headers=headers, params=params_cmd)
            print(f"Station {station_id} → {r.status_code}")

            # Commande prête
            if r.status_code == 200:
                js = r.json()
                if isinstance(js, list) and len(js) > 0:
                    id_cmde = js[0].get('idCommande')
                    print(f"   ✔ Commande prête : {id_cmde}")
                    break
                else:
                    print("   ⚠ Réponse vide, skip station")
                    id_cmde = None
                    break
        ...
        # Si aucune commande utilisable
        if not id_cmde:
            print(f"⛔ Pas de commande valide pour {station_id}, on continue.")
            continue

        # --- 2) RÉCUPÉRATION DU FICHIER ---
        params_file = {"id-cmde": id_cmde}
        file_req = requests.get(base_file_url, headers=headers, params=params_file)

        station_name = stations_df.loc[stations_df['id'] == station_id, 'nom'].values[0]
        station_name = station_name.replace(" ", "_")

        path = f"tmp/{dep}_{station_name}_2025_07+.csv"
        ...
        # Limiter les appels API pour éviter 429
        time.sleep(1)

    ti.xcom_push("meteo_paths", all_paths)
